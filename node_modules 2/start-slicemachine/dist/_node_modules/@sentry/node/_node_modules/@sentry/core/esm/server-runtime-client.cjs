...
class ServerRuntimeClient extends baseclient.BaseClient {
  ...
  eventFromException(exception, hint) {
    return syncpromise.resolvedSyncPromise(eventbuilder.eventFromUnknownInput(hub.getCurrentHub, this._options.stackParser, exception, hint));
  }
  /**
   * @inheritDoc
   */
  ...
  captureCheckIn(checkIn, monitorConfig, scope) {
    const id = checkIn.status !== "in_progress" && checkIn.checkInId ? checkIn.checkInId : misc.uuid4();
    if (!this._isEnabled()) {
