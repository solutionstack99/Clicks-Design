...
const FILE_CONTENT_CACHE = new lru.__exports.LRUMap(100);
...
class ContextLines {
  ...
  async addSourceContext(event) {
    ...
    if (this._contextLines > 0 && _optionalChain._optionalChain([event, "access", (_2) => _2.exception, "optionalAccess", (_3) => _3.values])) {
    ...
    if (this._contextLines > 0 && _optionalChain._optionalChain([event, "access", (_6) => _6.exception, "optionalAccess", (_7) => _7.values])) {
      for (const exception of event.exception.values) {
  ...
  addSourceContextToFrames(frames) {
    for (const frame of frames) {
      if (frame.filename && frame.context_line === void 0) {
        ...
        if (sourceFileLines) {
          try {
            misc.addContextToFrame(sourceFileLines, frame, this._contextLines);
          } catch (e) {
          }
        }
      }
