...
const index = require("../../../../https-proxy-agent/dist/index.cjs");
...
function makeNodeTransport(options) {
  let urlSegments;
  try {
    urlSegments = new require$$2.URL(options.url);
  } catch (e) {
    console.warn(
  ...
  const agent = proxy ? new index.dist.HttpsProxyAgent(proxy) : new nativeHttpModule.Agent({ keepAlive, maxSockets: 30, timeout: 2e3 });
...
function createRequestExecutor(options, httpModule, agent) {
  const { hostname, pathname, port, protocol, search } = new require$$2.URL(options.url);
